...
$._datagrid = function (p, options) {
    options = options || {};
    options.fit = options.fit || true;
    options.method = options.method || 'GET';
    options.pageSize = options.pageSize || 20;
    options.striped = options.striped || false;
    options.remoteSort = options.remoteSort || false;
    options.rownumbers = options.rownumbers || true;
    options.pagination = options.pagination || true;
    options.fitColumns = options.fitColumns || true;
    options.singleSelect = options.singleSelect || true;
    options.checkOnSelect = options.checkOnSelect || true;
    options.selectOnCheck = options.selectOnCheck || true;
